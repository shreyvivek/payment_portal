...
import { Check, Sparkles, CreditCard, Calendar, MapPin, Phone, AtSign, University } from "lucide-react";
...
/**
 * NTU Dandiya Registration – Polished, mobile‑friendly, Safari‑friendly
 * --------------------------------------------------------------------
 * • Theme refresh: warm terracotta gradient, soft glass cards, subtle festive pattern
 * • Mobile‑first layout, bigger tap targets, responsive grid
 * • Pricing logic UPDATED (per your note):
 *    - NTU: ALWAYS $6 (never surges)
 *    - Non‑NTU: $10 until NON‑NTU signups reach 50 → then $12
 * • QR images: NTU uses $6 QR always; Non‑NTU switches QR at surge
 * • Google Sheets logging via Apps Script (front‑end POST is Safari‑safe)
 */
...
const CONFIG = {
  pricing: {
    ...
    OTHER_BASE: 10,
    OTHER_SURGE: 12,
    NON_NTU_THRESHOLD: 50, // when non‑NTU signups >= 50, others pay 12
  },
  images: {
    ntuLogo: "/ntu-logo.jpeg", // put in /public
    bgLeft:
      "https://images.unsplash.com/photo-1572099606223-6e29045d8c92?q=80&w=1200&auto=format&fit=crop",
    bgRight:
      "https://images.unsplash.com/photo-1601134467661-3d775b999c8f?q=80&w=1200&auto=format&fit=crop",
    paynowNTU: "/paynow-ntu-6.jpg",
    paynowOther: "/paynow-other-10.jpg",
    paynowOtherSurge: "/paynow-other-12.jpg",
  },
  event: {
...
function computePrice(isNTU: boolean, nonNtuCount: number) {
  if (isNTU) return CONFIG.pricing.NTU; // always 6
...
export default function DandiyaRegistrationApp() {
  ...
  return (
    <div className="min-h-screen w-full relative overflow-x-hidden text-amber-50">
      {/* Background: layered warm gradient + subtle radial highlights */}
      ...
      {/* Decorative images, softened */}
      <img src={CONFIG.images.bgLeft} alt="left pattern" className="absolute -top-16 -left-8 w-[36rem] opacity-15 mix-blend-overlay select-none pointer-events-none" />
      <img src={CONFIG.images.bgRight} alt="right pattern" className="absolute -bottom-24 -right-10 w-[38rem] opacity-15 mix-blend-overlay select-none pointer-events-none" />
        ...
        <div className="rounded-3xl bg-white/10 backdrop-blur-md border border-white/15 px-4 sm:px-6 py-4 sm:py-6 shadow-xl">
          <div className="flex items-start sm:items-center justify-between gap-3 flex-col sm:flex-row">
            <div className="flex items-center gap-3 sm:gap-4">
              <img src={CONFIG.images.ntuLogo} alt="NTU" className="h-10 sm:h-12 w-auto rounded-md bg-white/80 p-1 shadow" />
              <div className="flex items-start gap-3">
                <motion.div initial={{ rotate: -15, scale: 0.9 }} animate={{ rotate: 0, scale: 1 }} transition={{ type: "spring" }} className="p-2 rounded-2xl bg-white/10">
            ...
            <div className="text-right text-[11px] sm:text-xs text-amber-100/90">
              <div>NTU <b>${CONFIG.pricing.NTU}</b> (fixed)</div>
              <div>Others <b>${CONFIG.pricing.OTHER_BASE}</b> → <b>${CONFIG.pricing.OTHER_SURGE}</b> after {CONFIG.pricing.NON_NTU_THRESHOLD} non‑NTU signups</div>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10 pb-24">
        ...
        <div className="grid grid-cols-3 gap-2 mb-5 sm:mb-8">
          {["Details", "Payment", "Done"].map((s, i) => {
            ...
            return (
              <div key={s} className={`h-2 rounded-full ${active ? "bg-yellow-400" : done ? "bg-emerald-400" : "bg-white/20"}`} />
            );
          })}
        </div>
        ...
        {step === "form" && (
          <Card className="bg-white/10 backdrop-blur-md text-amber-50 border border-white/15 shadow-2xl rounded-3xl">
            <CardHeader>
              <CardTitle className="text-xl sm:text-2xl">Tell us about you ✨</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="grid sm:grid-cols-2 gap-4 sm:gap-6">
                <div>
                  <Label htmlFor="name">Full Name</Label>
                  <Input id="name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mt-2 bg-white/10 border-white/20" required />
                </div>
                <div>
                  <Label htmlFor="telegram" className="flex items-center gap-2"><AtSign className="h-4 w-4" /> Telegram ID</Label>
                  <Input id="telegram" value={form.telegram} onChange={(e) => setForm({ ...form, telegram: e.target.value })} className="mt-2 bg-white/10 border-white/20" required />
                </div>
                <div>
                  <Label htmlFor="phone" className="flex items-center gap-2"><Phone className="h-4 w-4" /> Phone Number</Label>
                  <Input id="phone" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="mt-2 bg-white/10 border-white/20" inputMode="tel" pattern="\+?\d{8,15}" placeholder="e.g., +65 91234567" required />
                  <p className="text-xs text-amber-200/80 mt-1">Use digits only, with optional +country code.</p>
                </div>
                <div>
                  <Label htmlFor="matric">Matriculation Number</Label>
                  <Input id="matric" value={form.matric} onChange={(e) => setForm({ ...form, matric: e.target.value })} className="mt-2 bg-white/10 border-white/20" required />
                </div>
                <div className="sm:col-span-2">
                  <Label htmlFor="university" className="flex items-center gap-2"><University className="h-4 w-4" /> University</Label>
                  <select id="university" value={form.university} onChange={(e) => setForm({ ...form, university: e.target.value })} className="mt-2 w-full rounded-xl bg-white/10 border border-white/20 px-3 py-2">
                ...
                <div className="sm:col-span-2">
                  <Label htmlFor="remarks">Any notes (optional)</Label>
                  <Textarea id="remarks" value={form.remarks} onChange={(e) => setForm({ ...form, remarks: e.target.value })} className="mt-2 bg-white/10 border-white/20" rows={3} />
                </div>
                <div className="sm:col-span-2 flex items-start gap-3 mt-1">
                  ...
                  <Label htmlFor="agree" className="text-sm text-amber-200/90">I agree that my details will be used for event registration and communication.</Label>
                </div>

                <div className="sm:col-span-2 flex items-center justify-between mt-2">
                  <div className="text-amber-200/90 text-xs sm:text-sm">Non‑NTU signups (local): <span className="font-semibold">{nonNtuCount}</span> / {CONFIG.pricing.NON_NTU_THRESHOLD}</div>
                  <div className="text-right">
                    <div className="text-amber-200/90 text-xs">Your ticket</div>
                    <div className="text-2xl sm:text-3xl font-extrabold tracking-tight text-yellow-300">${price}.00</div>
                    <div className="text-[11px] text-amber-200/80">{isNTU ? "NTU student tier" : nonNtuCount >= CONFIG.pricing.NON_NTU_THRESHOLD ? "Non‑NTU surge" : "Non‑NTU early"}</div>
                  </div>
                </div>

                <div className="sm:col-span-2 flex gap-3 justify-end">
                  <Button type="submit" disabled={!valid} className="w-full sm:w-auto px-6 py-4 text-base rounded-2xl bg-yellow-400 text-red-900 hover:bg-yellow-300 transition">
            ...
            <CardContent>
              <div className="grid md:grid-cols-2 gap-6 items-start">
                <div className="flex flex-col items-center">
                  <img src={qrSrc} alt="PayNow QR" className="w-60 h-60 sm:w-64 sm:h-64 rounded-2xl shadow-2xl border border-amber-100/20 object-contain bg-white" />
                  <div className="text-xs text-amber-200/80 mt-2">QR shown for: <b>{isNTU ? "NTU ($6)" : price === CONFIG.pricing.OTHER_SURGE ? "Non‑NTU ($12)" : "Non‑NTU ($10)"}</b></div>
                </div>
                <div className="space-y-3">
                  <p className="text-amber-100/90">Please scan the QR and pay <span className="font-bold">${price}.00</span>. Use the reference below so we can match your payment quickly.</p>
                  <div className="p-4 rounded-xl bg-[#3f1a12]/50 border border-amber-100/20">
                    ...
                    <div className="flex items-center justify-between mt-1">
                      ...
                      <Button variant="secondary" className="rounded-xl" onClick={() => navigator.clipboard.writeText(paymentRef)}>Copy</Button>
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-end gap-3 mt-6">
                <Button variant="outline" className="rounded-2xl border-amber-100/30 text-amber-100" onClick={() => setStep("form")}>Back</Button>
                <Button className="w-full sm:w-auto px-6 py-4 text-base rounded-2xl bg-emerald-400 text-emerald-950 hover:bg-emerald-300" onClick={handlePaidAndConfirm}>
            ...
            <CardContent className="space-y-5">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-amber-100/90 mt-2 text-sm">We’ve received your registration. You’ll get a confirmation message via Telegram/WhatsApp within 48 hours after we verify your payment.</p>
                ...
                <div>
                  ...
                  <div className="mt-4 text-xs text-amber-200/80">Need help? DM <span className="font-medium">@ntudandiya</span> on Telegram.</div>
                </div>
              </div>
            </CardContent>
          </Card>
